...
public class CreepDeath : Projectile
{
    //creepManager
    ...
    public override void InitiateProjectile(GameObject origin)
    {
        ...
        pSystemExplosion.transform.GetComponent<Renderer>().material.color = origin.transform.GetComponent<Renderer>().material.color;
        mainShape.mesh = origin.transform.GetComponent<MeshFilter>().mesh;
        this.gameObject.SetActive(true);
        currentLifeTime = 10f;
        dimishing = false;
        PlaySFX(4, Vector3.Distance(player.transform.position, this.transform.position), .35f);
    }
